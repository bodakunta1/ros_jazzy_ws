...
private:
    ...
    void call_set_led(int64_t led_number, int64_t led_state)
    {
        ...
        auto future = client_->async_send_request(              // Create a service request to set_led service in led_panel node asynchronously
            request,                                            // Pass the request to the service
            [this, led_number, led_state](rclcpp::Client<my_robot_interfaces::srv::SetLed>::SharedFuture response) {                // Define a callback which is lambda function in here [this, led_number, led_state] to handle the response from the service
                handle_set_led_response(response, led_number, led_state);               //After reponse arrives, lambda call member function handle_set_led_response with repsonse, led_number and led_state as arguments
            }
        );
    }
